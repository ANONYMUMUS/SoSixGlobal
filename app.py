...
SECRET_V3_SCRIPT = r"""
-- [[ SONIX PRECISION v3: FULL INTEGRATED CLIENT ]]
-- [[ STATUS: 300+ LINES | FULL RECONSTRUCTION ]]
...
-- [[ 5. SEARCH & INPUT ]]
local SearchBar = Instance.new("Frame", MainContainer); SearchBar.Size = UDim2.new(0, 120, 0, 20); SearchBar.Position = UDim2.new(1, -130, 0, 5); SearchBar.BackgroundColor3 = Color3.fromRGB(25, 25, 25); SearchBar.Visible = false; SearchBar.ZIndex = 10; Instance.new("UICorner", SearchBar).CornerRadius = UDim.new(0, 6)
...
local InputBox = Instance.new("TextBox", InputBar); InputBox.Size = UDim2.new(1, -40, 1, 0); InputBox.BackgroundColor3 = Color3.fromRGB(20,20,20); InputBox.TextColor3 = Color3.new(1,1,1); InputBox.PlaceholderText = "Message..."; InputBox.Text = ""; InputBox.ZIndex = 9; InputBox.TextEditable = true; Instance.new("UICorner", InputBox); Instance.new("UIPadding", InputBox).PaddingLeft = UDim.new(0, 10)
...
-- [[ 6. PROFILE SYSTEM ]]
local InfoLayer = Instance.new("Frame", ScreenGui); InfoLayer.Size = MainContainer.Size; InfoLayer.Position = ChatVisiblePos; InfoLayer.BackgroundTransparency = 1; InfoLayer.Visible = false; InfoLayer.ZIndex = 50 
local BluffOverlay = Instance.new("Frame", InfoLayer); BluffOverlay.Size = UDim2.new(1, 0, 1, 0); BluffOverlay.BackgroundColor3 = Color3.new(0, 0, 0); BluffOverlay.BackgroundTransparency = 1; BluffOverlay.ZIndex = 51; Instance.new("UICorner", BluffOverlay).CornerRadius = UDim.new(0, 20)
...
local function CreateInfoBox(label, value, targetPos, delay)
    ...
    local box = Instance.new("TextButton", InfoLayer); box.Size = UDim2.new(0, 85, 0, 28); box.Position = targetPos - UDim2.new(0, 0, 0, 20); box.BackgroundColor3 = Color3.fromRGB(45, 45, 45); box.BackgroundTransparency = 1; box.ZIndex = 60; box.Text = ""; Instance.new("UICorner", box).CornerRadius = UDim.new(1, 0)
    local txt = Instance.new("TextLabel", box); txt.Size = UDim2.new(1, 0, 1, 0); txt.BackgroundTransparency = 1; txt.RichText = true; txt.Text = "<b>"..label.."</b>\n"..tostring(value); txt.TextColor3 = Color3.new(1, 1, 1); txt.TextSize = 7; txt.ZIndex = 61
...
local function Inspect(uid, name)
    ...
    TweenService:Create(BluffOverlay, TweenInfo.new(0.3), {BackgroundTransparency = 0.4}):Play() 
    local age = 0; pcall(function() age = (Players:GetPlayerByUserId(uid) or {AccountAge = 0}).AccountAge end)
    local midPfp = Instance.new("ImageLabel", InfoLayer); midPfp.Size = UDim2.new(0, 40, 0, 40); midPfp.Position = UDim2.new(0.5, -20, 0.5, -20); midPfp.Image = Players:GetUserThumbnailAsync(uid, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size48x48); midPfp.BackgroundTransparency = 1; midPfp.ZIndex = 60; Instance.new("UICorner", midPfp).CornerRadius = UDim.new(1, 0); table.insert(ActiveBoxes, midPfp)
...
-- [[ 7. MENU SYSTEM ]]
local MenuContainer = Instance.new("CanvasGroup", ScreenGui); MenuContainer.Size = UDim2.new(0, 100, 0, 220); MenuContainer.Position = MenuHiddenPos; MenuContainer.BackgroundTransparency = 1; MenuContainer.GroupTransparency = 1; MenuContainer.Visible = false; MenuContainer.ZIndex = 5
...
local function LoadFriends()
    ...
    UpdateLayout("FRIENDS"); AllFriendsCache = {}
    task.spawn(function()
...
local function AddMsg(scroll, uid, user, text)
    ...
    task.defer(function() scroll.CanvasPosition = Vector2.new(0, 9999) end)
end

local function Send(txt)
    if not canSend or txt == "" then return end
    canSend = false; SendBtn.TextColor3 = Color3.fromRGB(80, 80, 80)
    local payload = HttpService:JSONEncode({["PlayerName"] = Player.Name, ["UserId"] = Player.UserId, ["Message"] = txt, ["Type"] = (CurrentMode == "DM" and "private" or "global"), ["TargetId"] = (CurrentMode == "DM" and CurrentDMTarget.UserId or nil)})
    pcall(function()
        if syn then syn.request({Url = SERVER_URL.."/send", Method = "POST", Body = payload, Headers = {["Content-Type"]="application/json"}})
        else HttpService:PostAsync(SERVER_URL.."/send", payload, Enum.HttpContentType.ApplicationJson) end
    end)
...
task.spawn(function()
    while task.wait(3) do
        local success, response = pcall(function() return HttpService:GetAsync(SERVER_URL.."/get_messages?after="..lastTimestamp) end)
        if success then
            ...
            for _, m in ipairs(messages) do 
                if m.Timestamp > lastTimestamp then 
                    ...
                    elseif m.Type == "private" and ((m.TargetId == Player.UserId and CurrentDMTarget and CurrentDMTarget.UserId == m.UserId) or (m.UserId == Player.UserId and CurrentDMTarget and CurrentDMTarget.UserId == m.TargetId)) then AddMsg(DMView, m.UserId, m.PlayerName, m.Message) end
                end 
            end
        end
    end
...
MainBtn.MouseButton1Click:Connect(function()
    if MainContainer.Visible then
        ToggleMenu(false); TweenService:Create(MenuContainer, TweenInfo.new(0.3), {Position = MenuHiddenPos, GroupTransparency = 1}):Play(); TweenService:Create(MainContainer, TweenInfo.new(0.3), {Position = ChatHiddenPos, GroupTransparency = 1}):Play(); task.wait(0.3); MainContainer.Visible = false; MenuContainer.Visible = false
    else
        HeaderTitle.Text = "GLOBAL FEED"; UpdateLayout("GLOBAL"); MainContainer.Visible = true; MenuContainer.Visible = true; TweenService:Create(MainContainer, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Position = ChatVisiblePos, GroupTransparency = 0}):Play(); TweenService:Create(MenuContainer, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Position = MenuVisiblePos, GroupTransparency = 0}):Play()
    end
end)

SendBtn.MouseButton1Click:Connect(function() if canSend and InputBox.Text ~= "" then local t = InputBox.Text; InputBox.Text = ""; Send(t) end end)
subButtons[1].MouseButton1Click:Connect(function() HeaderTitle.Text = "GLOBAL FEED"; UpdateLayout("GLOBAL"); ToggleMenu(false) end)
subButtons[2].MouseButton1Click:Connect(function() LoadFriends(); ToggleMenu(false) end)
subButtons[3].MouseButton1Click:Connect(function() Inspect(Player.UserId, Player.Name); ToggleMenu(false) end)
BluffOverlay.InputBegan:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 then ClearInfo() end end)"""
