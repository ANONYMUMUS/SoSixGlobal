...
# --- CONFIG ---
# Added lowercase version for safety
...
def send_message():
    ...
    if p_type:
        return jsonify({"status": "restricted", "type": p_type}), 403
...
def get_messages():
    try:
        ...
        with get_db_connection() as conn:
            # FIX: Explicitly convert SQLite rows to Dictionaries
        ...
        print(f"CRITICAL ERROR in get_messages: {e}")
        return jsonify([]), 500

@app.route('/admin/execute', methods=['POST'])
def execute_command():
    data = request.json
    # Convert mod_name to lower so "Shadowss_99" and "shadowss_99" both work
    mod_name = str(data.get('ModName', "")).lower()
    target_name = data.get('TargetName')
    target_uid = data.get('TargetId')
    action = data.get('Action')
    duration = data.get('Duration', 0)

    if mod_name not in [m.lower() for m in MODERATORS]:
        print(f"Refused Admin access to: {mod_name}")
