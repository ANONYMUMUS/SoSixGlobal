...
# [[ THE EXACT V3 SCRIPT YOU PROVIDED - FIXED URLS ]]
SECRET_V3_SCRIPT = r"""
...
-- [[ 7. PROFILE SYSTEM ]]
local InfoLayer = Instance.new("Frame", ScreenGui); InfoLayer.Size = MainContainer.Size; InfoLayer.Position = ChatVisiblePos; InfoLayer.BackgroundTransparency = 1; InfoLayer.Visible = false; InfoLayer.ZIndex = 20
...
local function LoadFriends()
    HeaderTitle.Text = "FRIENDS"; HeaderTitle.TextColor3 = Color3.fromRGB(0, 255, 150); UpdateLayout("FRIENDS"); AllFriendsCache = {}
    task.spawn(function()
        ...
        if success then
            while true do
                ...
                if pages.IsFinished then break end; pages:AdvanceToNextPageAsync()
            end; RenderFriendList(nil)
        end
    end)
end
...
local function Send(txt)
    ...
    local payload = {["PlayerName"] = Player.Name, ["UserId"] = Player.UserId, ["Message"] = txt, ["Type"] = (CurrentMode == "DM" and "private" or "global"), ["TargetId"] = (CurrentMode == "DM" and CurrentDMTarget.UserId or nil)}
            ...
            for _, m in ipairs(messages) do 
                if m.Timestamp > lastTimestamp then 
                    ...
                    elseif m.Type == "private" then
                        if (m.TargetId == Player.UserId and CurrentDMTarget and CurrentDMTarget.UserId == m.UserId) or (m.UserId == Player.UserId and CurrentDMTarget and CurrentDMTarget.UserId == m.TargetId) then
...
MainBtn.MouseButton1Click:Connect(function()
    if MainContainer.Visible then
        ...
        HeaderTitle.Text = "GLOBAL FEED"; UpdateLayout("GLOBAL"); MainContainer.Visible = true; TweenService:Create(MainContainer, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Position = ChatVisiblePos, GroupTransparency = 0}):Play(); task.wait(0.2); MenuContainer.Visible = true; TweenService:Create(MenuContainer, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Position = MenuVisiblePos, GroupTransparency = 0}):Play()
    end
end)
"""
...
def send_message():
    ...
    msg = {
        ...
        "Type": data.get("Type", "global"), # FIXED: Store the type
        "TargetId": data.get("TargetId"),   # FIXED: Store the target
        "Timestamp": time.time()
    }
    messages.append(msg)
...
def get_messages():
    ...
    # Send back all messages; the Lua script now correctly filters them
